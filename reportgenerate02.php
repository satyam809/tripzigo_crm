...
function getQueriesForMonth($date)
{
    ...
    (SUM(CASE WHEN qStM.name = 'Confirmed' THEN 1 ELSE 0 END) / SUM(CASE WHEN qM.assignTo IS NOT NULL THEN 1 ELSE 0 END)) * 100 AS `Confirmed %`,
    FORMAT(SUM(spB.grossPrice), 0) AS revenue,
    FORMAT(SUM(spB.netPrice), 0) AS COGS,
    FORMAT(SUM(spB.grossPrice - spB.netPrice), 0) AS Profit,
    SUM(spB.grossPrice - spB.netPrice)/SUM(spB.netPrice) * 100 AS `Profit %`
FROM
queryMaster AS qM
INNER JOIN queryStatusMaster AS qStM ON qM.statusId = qStM.id
INNER JOIN querySourceMaster AS qSM ON qM.leadSource = qSM.id
INNER JOIN sys_userMaster AS s_uM ON qM.assignTo = s_uM.id
LEFT JOIN sys_packageBuilder AS spB ON qM.id = spB.queryId
WHERE
...
with ROLLUP;";
    ...
    for ($i = 0; $i < count($queries1); $i++) {
        if ($queries1[$i]['Assigned To (Id)'] == '1') {
            $queries1[$i]['Assigned To'] = 'Not Assigned';
        }

        if ($queries1[$i]['Assigned To (Id)'] == 'Total') {
            $queries1[$i]['Assigned To'] = 'Total';
        }
    }
    ...
    foreach ($queries1 as &$row) {
        // $row['revenue'] = formatIndianNumber($row['revenue']);
        // $row['COGS'] = formatIndianNumber($row['COGS']);
        // $row['Profit'] = formatIndianNumber($row['Profit']);
        $row['Confirmed %'] = number_format($row['Confirmed %'], 2) . '%';
        $row['Profit %'] = number_format($row['Profit %'], 2) . '%';
        
    }
    
    // $totalIndex = count($queries1) - 1;

    // usort($queries1, function($a, $b) use ($totalIndex) {
...
// $totalRow = $queries1[$totalIndex];
// unset($queries1[$totalIndex]);
// $queries1[] = $totalRow;
...
// echo '<pre>';
// print_r($result);
send_report_mail('info@awtrips.com', 'pradeep@tripzygo.in', 'Daily GPM status report - '.$date, $mailbody, 'neerajvani@tripzygo.in, simranvani@tripzygo.in, nitinrana@tripzygo.in, manishyadav@tripzygo.in', '');
