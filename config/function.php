...
function deleteFiles($select, $tablename, $where)

{
  ...
  while ($data = mysqli_fetch_array($rs)) {
    ...
    unlink('collectionphotos/' . $data[$select]);
  }
}
...
function deleteitineraryFiles($select, $tablename, $where)

{
  ...
  while ($data = mysqli_fetch_array($rs)) {
    ...
    unlink('package_image/' . $data[$select]);
  }
}
...

{
  ...
  if ($number > 0) {
    ...
    $sql = "INSERT INTO active_sessions (user_id, session_id) VALUES ('$userId', '$sessionId')";
...
function addlistingCalldesk($tablename, $namevalue)
...

{
  ...
  if ($id != '') {
    ...
    $rsaa = GetPageRecord($selectaa, 'expenseTypeMaster', $whereaa);
...
//----------------Get Total Value Of Specific Column---------------------//

function GetTotalOfColumn($select, $tablename, $where)

{

  $sql = "select SUM(" . $select . ')' . " from " . $tablename . " where " . $where . "";
...
//----------------Get Status For Payment Links---------------------//

function GetStatusForPaymentLinks($id)

{
  ...
  $data = GetPageRecord($select, 'queryPaymentLinks', $where);

  while ($response = mysqli_fetch_array($data)) {
    ...
    return '<span class="badge ' . $classnew . '">' . $msg . '</span>';
  }
}

function GetStatusForSysPackageBuilder($id)

{
  ...
  $data = GetPageRecord($select, 'sys_PackagePayment', $where);
