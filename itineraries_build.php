<?php
    $totalPax = stripslashes($result['adult']) + stripslashes($result['child']); ?>
    <?php
    ...
    <div style="padding:20px; padding-left:80px;">
            ...
            <tr>
                <td colspan="2" align="left" valign="top" style="border:1px solid #ecf0f2;">
                    <div style="height:150px; overflow:hidden; position:relative;" class="coverBanner">
                        <img src="<?php echo $fullurl; ?>package_image/<?php echo str_replace('', '', $result['coverPhoto']); ?>"
                             style="width:100%; height:auto; min-height:100%;"/>
                        <div style="background-color: #000000ba; color: #fff; padding: 10px 20px; font-size: 25px; position: absolute; left: 0px; bottom: 0px; width: 100%; font-weight: 600;"><?php echo stripslashes($result['name']); ?>
                            <a style="font-size:18px; cursor:pointer;" onclick="loadpop2('Itinerary setup',this,'600px')"
                               data-toggle="modal" data-target="#myModal2" data-backdrop="static"
                               popaction="action=addtineraries&amp;id=<?php echo encode($result['id']); ?>">&nbsp;&nbsp;<i
                        ...
                        <a style="font-size: 13px; background-color: #00000082; color: #fff; cursor: pointer; position: absolute; right: 10px; top: 10px; padding: 5px 10px;border-radius: 4px;"
                           ...
                           popaction="action=medialibrary&amp;afunctin=changeCoverPhoto&amp;pid=<?php echo encode($result['id']); ?>"><i
                                    class="fa fa-picture-o" aria-hidden="true"></i> &nbsp;Change Cover Photo</a>
                    </div>
                        ...
                        <tr>
                            <td width="25%" colspan="2" align="left" valign="top" style="border-right:1px solid #ecf0f2;">
                                ...
                                for ($i = $begin; $i <= $end; $i->modify('+1 day')) {
                                    ...
                                    <div class="itidaytab" id="dayid<?php echo $n; ?>"
                                         onclick="load_build_day_details('<?php echo $n; ?>','<?php echo date('Y-m-d', strtotime($i->format("Y-m-d"))); ?>');">

                                        <?php if ($result['queryId'] > 0) { ?>
                                            <strong><span><?php echo $n; ?></span> <?php echo $i->format("d"); ?> <?php echo $i->format("M"); ?>
                                                - <?php echo date('D', strtotime($i->format("Y-m-d"))); ?></strong>
                                        ...
                                        <select id="destinationName<?php echo $n; ?>" class="form-control"
                                                onchange="load_build_day_details('<?php echo $n; ?>','<?php echo date('Y-m-d', strtotime($i->format("Y-m-d"))); ?>');">
                                            <?php foreach (explode(',', $result['destinations']) as $val) {
                                                if ($val != '') { ?>
                                                    <option value="<?php echo $val; ?>"
                                                            <?php if ($checkday['destinationName'] == $val) { ?>selected="selected" <?php } ?>><?php echo $val; ?></option>
                                                <?php }
                                            } ?>
                                        </select>
                                    </div>
                                ...
                                <div class="itidaytab" id="dayid100000"
                                     onclick="load_build_day_details('100000','<?php echo date('Y-m-d', strtotime($i->format("Y-m-d"))); ?>');">

                                    <strong><i class="fa fa-file-text-o" aria-hidden="true"></i> &nbsp; Package
                                        Terms</strong>
                                ...
                                <div class="itidaytab" id="dayid110000"
                                     onclick="load_build_day_details('110000','<?php echo date('Y-m-d', strtotime($i->format("Y-m-d"))); ?>');">


                                    <strong><i class="fa fa-image" aria-hidden="true"></i> &nbsp; Slider Images</strong>
                                ...
                                <div class="itidaytab" id="dayid120000"
                                     onclick="load_build_day_details('120000','<?php echo date('Y-m-d', strtotime($i->format("Y-m-d"))); ?>');">


                                    <strong><i class="fa fa-file-text-o" aria-hidden="true"></i> &nbsp; FAQs</strong>
                            ...
                            <td align="left" valign="top">
                                <div id="load_build_day_details"></div>
                            </td>
                        </tr>
                    </table>
        ...
        <div class="systemcard" id="followupsec">
            ...
            <div class="row">
                ...
                while ($itinerary_image_detail = mysqli_fetch_array($itinerary_image_rs)) {
                    ...
                    <div class="mx-2 my-2">
                        <img src="package_image/<?php print_r($itinerary_image_detail['image_path']); ?>" alt="Snow"
            ...
            <div class="m-3">
                <!--<b class="pr-3">Hotel Star</b>-->
                 <b class="pr-3">Hotel/Package <i class="fa fa-star-o">
                </i> Rating</b>
                <?php
                if ($result['hotel'] == 0) {
                    ?>
            ...
            if ($result['aboutPackage'] != "") {
                ?>
                <div class="m-3">
                    <b class="pr-3">About Package</b>
                    <span><?php echo $result['aboutPackage']; ?></span>
                </div>
                <?php
            }
             if ($result['web_pack_price'] != 0) {
                ?>
                <div class="m-3">
                    <b class="pr-3">Package Price</b>
                    <span>INR 
                    <?php
                    setlocale(LC_MONETARY, 'en_IN');
                  echo money_format('%!i', $result['web_pack_price']); ?></span>
                </div>
                <?php
             }
            ...
            <div class="m-3">
                ...
                <span class="badge badge-info"><?php if ($result['publicly_visible']) {
                        echo "Yes";
                    } else {
                        echo "No";
                    } ?></span>
            ...
            <div class="m-3">
                <b class="pr-3">Website Visible</b>
                <span class="badge badge-info"><?php if ($result['website_visible']) {
                        echo "Yes";
                    } else {
                        echo "No";
                    } ?></span>
        ...
        <script>
            ...
            function uploadSliderImage(input,dataIndx,file_data,itnryId) {
                if (input.files && input.files[0]) {
                    $('#imagePreview'+dataIndx).html('<img alt="default-image" src="images/ajax-loader.gif">');
                    var altText = $('#imgAlt'+dataIndx).val();

                    var form_data = new FormData();
                    form_data.append('file',file_data);
                    form_data.append('si_indx',dataIndx);
                    form_data.append('itnryId',itnryId);
                    form_data.append('altText',altText);

                    $.ajax({
                        url: 'slider_image_upload.php', // <-- point to server-side PHP script
                        ...
                        success: function(resp){
                            ...
                            if(resObj.status) {
                                ...
                                reader.onload = function (e) {
                                    $('#imagePreview' + dataIndx).html('<img alt="selected-image" src="' + e.target.result + '">');
                                    $('#imagePreview' + dataIndx).hide();
                                    $('#imagePreview' + dataIndx).fadeIn(1000);
                                };
                                reader.readAsDataURL(input.files[0]);
                            }
                            else{
                                if($.trim(resObj.savedImg) !== '') {
                                    $('#imagePreview' + dataIndx).html('<img alt="selected-image" src="package_image/' + resObj.savedImg + '">');
                                }
                                else{
                                    $('#imagePreview'+dataIndx).html('<img alt="default-image" src="images/default-image.png">');
                                }

                                Swal.fire("Error", resObj.msg, "error");
                            }
                        }
                    });
                }
            ...
            $(document).on("change",".sliderPhotos",function() {
                var file_data = $(this).prop('files')[0];
                var itnryId = "<?php echo  decode($_REQUEST['id']) ?>";
                uploadSliderImage(this,$(this).data('inx'),file_data,itnryId);
            });

            $(document).on('blur','.alt_text',function () {
                var altText = $(this).val();
                var altIndx = $(this).data('altinx');
                var itnryId = "<?php echo  decode($_REQUEST['id']) ?>";
                if(($.trim(altText) !== '') && (parseInt(altIndx) > 0)){
                    console.log(altText);
                    console.log(altIndx);
                    ...
                    Swal.fire("Error", "Alt Text Cannot Be Blank!", "error");
                }
            });
